# php-react — PHP + React reference app

A minimal two-process "hello world" — **PHP** API + **React** front
end — that fetches and displays `GET /api/hello`.

## Layout (under `coderflow-reference-apps/php-react/`)
...
- **Back-end** edits: the PHP built-in server re-reads files per request, so backend changes show on the next request — no restart needed.
