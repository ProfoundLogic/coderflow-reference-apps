# dotnet-angular — .NET + Angular reference app

A minimal two-process "hello world" — **.NET** API + **Angular** front
end — that fetches and displays `GET /api/hello`.

## Layout (under `coderflow-reference-apps/dotnet-angular/`)
...
- **Back-end** edits: the API runs under `dotnet watch`, so saving a backend file rebuilds and restarts it automatically.
