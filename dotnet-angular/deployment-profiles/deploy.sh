#!/usr/bin/env bash
#
# Reference Deploy Profile — .NET + Angular
# WHAT THIS SHOWS
...
#   Angular front end (static build) plus the .NET API
#   (framework-dependent publish output), assembled into release.tgz.
#
...
APP_DIR="/workspace/coderflow-reference-apps/dotnet-angular"
TARGET="${TARGET:-qa}"
DRY_RUN="${DRY_RUN:-true}"

echo "=================================================="
echo " CoderFlow reference deploy — .NET + Angular"
echo "   Target:   ${TARGET}"
...
echo "    On the target, the API would start with: dotnet Api.dll"
