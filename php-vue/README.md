...
1. **Import Environment → Git repository**, paste this repo's URL, **Load environments**, pick `php-vue`, **Import**.
2. Build and launch it. The pre-clone script installs **PHP 8.x (php-cli)**, the post-clone action installs dependencies, and the application server starts the API (port 3001) and the Vue dev server (port 5173).
...
from Vue, the API's message ("Hello from the PHP API!") fetched
