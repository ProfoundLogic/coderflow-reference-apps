# node-angular — Node.js + Angular reference app

A minimal two-process "hello world" — **Node.js** API + **Angular** front
end — that fetches and displays `GET /api/hello`.

## Layout (under `coderflow-reference-apps/node-angular/`)
...
- **Back-end** edits: the API runs under `node --watch`, so saving a backend file restarts it automatically.
