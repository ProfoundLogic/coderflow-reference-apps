...
1. **Import Environment → Git repository**, paste this repo's URL, **Load environments**, pick `java-angular`, **Import**.
2. Build and launch it. The pre-clone script installs **JDK 21 (Temurin) + Maven**, the post-clone action installs dependencies, and the application server starts the API (port 3001) and the Angular dev server (port 4200).
...
from Angular, the API's message ("Hello from the Java API!") fetched
